  ...
  try{
    if(file.name.toLowerCase().endsWith('.pdf')){
      ...
      for(let i=1;i<=pdf.numPages;i++){
        ...
        // Extract preserving exact positions for accurate reconstruction
        const items=ct.items.filter(x=>x.str&&x.str.trim());
        if(!items.length){pages.push('');continue;}
        // Sort by Y (desc) then X (asc) to read top-to-bottom, left-to-right
        ...
        for(const item of sorted){
          ...
          if(curY===null||Math.abs(y-curY)>4){
            if(curLine.length)pageLines.push(curLine);
            curLine=[{x:item.transform[4],str:item.str}];
            curY=y;
          }else{
            curLine.push({x:item.transform[4],str:item.str});
          }
        }
        if(curLine.length)pageLines.push(curLine);
        // Build text from sorted lines
        const lineStrs=pageLines.map(line=>{
          line.sort((a,b)=>a.x-b.x);
          return line.map(x=>x.str).join(' ');
        });
        pages.push(lineStrs.join('\n'));
      }
      let txt=pages.join('\n\n--- PÁGINA ---\n\n');
      // Normalize: collapse 3+ spaces, trim lines
      txt=txt.split('\n').map(l=>l.replace(/  +/g,' ').trim()).join('\n');
      S.texts[idx]=txt.trim();
      S.rawTexts=S.rawTexts||[];
      S.rawTexts[idx]=txt.trim();
...
${filled}
${empty?'NOTA: Estos campos NO fueron proporcionados, NO los evalúes: '+empty:''}
...
- contratoAnotado: el texto COMPLETO del contrato marcando con [ERROR:fragmento], [FALTANTE:descripcion], [OK:fragmento] solo los fragmentos relevantes
- puntaje: 0-100 (100 = todo coincide perfectamente, penaliza solo errores reales y faltantes confirmados)\`;t="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Verificador de Contratos — Portalia Desarrollos Inteligentes</title>
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
<script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"></script>
